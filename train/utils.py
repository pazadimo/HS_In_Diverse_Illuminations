...
def save_checkpoint_fruit(model_path, epoch, iteration, model, optimizer):
    """Save the checkpoint."""
    state = {
            'epoch': epoch,
            'iter': iteration,
    ...
    torch.save(state, os.path.join(model_path, 'HS_Fruit_%d.pkl' %(epoch)))

def save_checkpoint_material(model_path, epoch, iteration, model, optimizer):
    """Save the checkpoint."""
    state = {
            'epoch': epoch,
            'iter': iteration,
    ...
    torch.save(state, os.path.join(model_path, 'HS_Material_%d.pkl' %(epoch)))
