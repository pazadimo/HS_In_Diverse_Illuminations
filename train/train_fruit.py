...
from utils import AverageMeter,initialize_logger,save_checkpoint_fruit,record_loss
...
def main():
    ...
    # Dataset
    train_data = DatasetFromHdf5('./Data/train_Fruit_.h5')
    print(len(train_data))
    val_data = DatasetFromHdf5('./Data/valid_Fruit_.h5')
    print(len(val_data))
    ...
    loss_csv = open(os.path.join(model_path,'loss_fruit.csv'), 'w+')
    
    log_dir = os.path.join(model_path,'train_fruit.log')
    ...
    for epoch in range(start_epoch+1, end_epoch):
        ...
        save_checkpoint_fruit(model_path, epoch, iteration, model, optimizer)     
